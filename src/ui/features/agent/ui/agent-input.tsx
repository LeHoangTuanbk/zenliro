...
export function AgentInput({ isStreaming, onSend, onStop }: AgentInputProps) {
  ...
  const handleKeyDown = (e: KeyboardEvent) => {
    // Stop all keyboard events from bubbling to canvas shortcuts
    e.stopPropagation();
  ...
  return (
    <div className="px-3 pb-3 pt-1 shrink-0">
      <div className="rounded-[8px] border border-[#333] bg-[#222] focus-within:border-[#555] transition-colors overflow-hidden">
        {/* Textarea */}
        <textarea
          ref={textareaRef}
          value={text}
        ...
        <div className="flex items-center justify-between px-2 pb-1.5">
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-[#777]">Claude</span>
            <ReferencePicker />
          </div>
